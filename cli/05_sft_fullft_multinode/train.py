#!/usr/bin/env python3
"""SFT full-parameter fine-tuning with DeepSpeed ZeRO-2 across MULTIPLE 8xH100
nodes (AI Runtime CLI, multi-node torchrun).

AIR-CLI version of `05.SFT Full-Parameter on Multi Node GPU with
DeepSpeedDistributer.py`. The classic version used pyspark
DeepspeedTorchDistributor with nnodes>1 on a Spark cluster; here AIR provisions
N nodes of 8xH100 and injects NUM_NODES / NODE_RANK / MASTER_ADDR / MASTER_PORT,
which torchrun uses for cross-node rendezvous. HF Trainer drives DeepSpeed
ZeRO-2. The training code itself is identical to the single-node version — only
the launcher (torchrun --nnodes) and the compute block differ.

NOTE (Azure): multi-node H100 is not yet available on Azure Serverless GPU as of
this writing; this multi-node path is validated on AWS.
"""
...
def main():
    ...
    if active is not None:
        ...
        mlflow.set_tag("task", "SFT full-parameter (DeepSpeed ZeRO-2, multi-node 8xH100, AIR CLI)")
        mlflow.log_params(
            {
